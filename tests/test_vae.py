...
# Visualize first benign update
from src.utils.visualize import visualize_update_reconstruction
from torch.serialization import add_safe_globals

# Add all required numpy functions to safe globals
add_safe_globals([
...
class TestVAEDefense(unittest.TestCase):
    def setUp(self):
        ...
        # Data directory
        self.data_dir = Path('data/updates')
        
        # Generate or load updates
        if not (self.data_dir / 'benign_updates.pt').exists():
            self.benign_updates, self.malicious_updates, self.mal_indices = \
                print('generate_and_save_updates')#generate_and_save_updates(self.total_params)
        else:
            self.benign_updates = torch.load(self.data_dir / 'benign_updates.pt', weights_only=False)
            self.malicious_updates = torch.load(self.data_dir / 'malicious_updates.pt', weights_only=False)
            self.mal_indices = torch.load(self.data_dir / 'malicious_indices.pt', weights_only=False)
            #self.benign_updates = torch.load(self.data_dir / 'benign_updates.pt')
            #self.malicious_updates = torch.load(self.data_dir / 'malicious_updates.pt')
            #self.mal_indices = torch.load(self.data_dir / 'malicious_indices.pt' )
    ...
    def test_visualization(self):
        ...
        visualize_update_reconstruction(self.vae, example_update)
